	...
	function nirmala_posted_on() {
		...
		$posted_on   = apply_filters(
			'nirmala_posted_on', sprintf(
				'<span class="posted-on"><i class="fa fa-clock-o" aria-hidden="true"></i> <a href="%1$s" rel="bookmark">%2$s</a></span>',
		...
		$byline      = apply_filters(
			'nirmala_posted_by', sprintf(
				'<span class="byline ml-12px"><i class="fa fa-user-circle-o" aria-hidden="true"></i> <span class="author vcard"><a class="url fn n" href="%1$s">%2$s</a></span></span>',
	...
	function nirmala_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && nirmala_categorized_blog() ) {
				...
				printf( '<span class="mr-12px"><i class="fa fa-folder-open-o" aria-hidden="true"></i> ' . $categories_list . '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'nirmala' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="mr-12px"><i class="fa fa-hashtag" aria-hidden="true"></i>' . $tags_list . '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="mr-12px"><i class="fa fa-comment-o" aria-hidden="true"></i> ';
