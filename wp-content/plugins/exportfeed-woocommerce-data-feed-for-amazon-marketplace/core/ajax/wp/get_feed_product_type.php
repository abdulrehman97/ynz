...
if (array_key_exists('tpl_id', $_POST) && isset($_POST['tpl_id'])) {
    $remote_tpl_id_for_feed_product_type = array_key_exists('remote_template_id', $_POST) ? $_POST['remote_template_id'] : '';
    ...
    if (is_array($data->results) && array_key_exists('0', $data->results)) {
        $feed_product_type = maybe_unserialize(utf8_decode($data->results[0]->valid_values));
    } else {
    ...
    $html = 'empty';
    $hidelabel = false;
    if (is_array($feed_product_type) && $feed_product_type !== false && count($feed_product_type) > 0 && strlen($feed_product_type[0]) > 0) {
...
switch ($country) {
    case 'US':
        if ($category_name == 'computers' || $category_name == 'consumerelectronics') {
            $flat_tmpl_id = 5;
        }

        if ($category_name == "industrial" || $category_name == 'mechanicalfasteners' || $category_name == 'labsupplies' || $category_name == 'powertransmission' || $category_name == 'rawmaterials' || $category_name == 'mechanicalfasteners') {
            $flat_tmpl_id = 11;
        }
    ...
    case 'UK':
        if ($category_name == 'consumerelectronics') {
            $flat_tmpl_id = 40;
        }

        if ($category_name == 'eyewear') {
            $flat_tmpl_id = 39;
        }
    ...
    case 'DE':
        if ($category_name == 'industrial' || $category_name == 'labsupplies') {
            $flat_tmpl_id = 95;
        }

        if ($category_name == 'lawnandgarden') {
            $flat_tmpl_id = 93;
        }
    ...
    case 'CA':
        if ($category_name == 'computers' || $category_name == 'wireless') {
            $flat_tmpl_id = 66;
        }
    ...
    case 'IT':
        ...
        //Same with Consumer Electornics
        if ($category_name == 'personalcareappliances') {
            $flat_tmpl_id = 172;
        }
