    ...
    class AMWSCP_PAmazonSCFeed extends AMWSCP_PCSVFeed
    {
        ...
        public function loadTemplate($template)
        {
            ...
            if ($template) {
                ...
                curl_close($ch);
                $tpl_columns = $data->results;
            }

            if (count($tpl_columns) > 0) {
                foreach ($tpl_columns as $key => $column) {

                    if ($column->fields == 'item_sku') {
                        $this->addAttributeMapping('sku', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'item_name') {
                        $this->addAttributeMapping('title', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'brand_name' ||
                        $column->fields == 'manufacturer'
                    ) {
                        $this->addAttributeMapping('brand', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'external_product_id' ||
                        $column->fields == 'product-id-number' ||
                        $column->fields == 'product-id'
                    ) {
                        $this->addAttributeMapping('upc', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'standard_price') {
                        $this->addAttributeMapping('regular_price', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'product_description') {
                        $this->addAttributeMapping('description_short', $column->fields, true, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'main_image_url') {
                        $this->addAttributeMapping('feature_imgurl', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'item_quantity' ||
                        $column->fields == 'quantity'
                    ) {
                        $this->addAttributeMapping('stock_quantity', $column->fields, false, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'item_length') {
                        $this->addAttributeMapping('length', $column->fields, false, $column->required)->localized_name = $column->labels;
                    } elseif ($column->fields == 'item_width') {
                        $this->addAttributeMapping('width', $column->fields, false, $column->required)->localized_name = $column->labels;
                    } elseif ($column->fields == 'item_height') {
                        $this->addAttributeMapping('height', $column->fields, false, $column->required)->localized_name = $column->labels;
                    } elseif ($column->fields == 'external_product_id_type' ||
                        $column->fields == 'product-id-number-type' ||
                        $column->fields == 'product-id-type'
                    ) {
                        $this->addAttributeMapping($column->fields, $column->fields, true)->localized_name = $column->labels;
                    } elseif ($column->fields == 'bullet_point1') {
                        $this->addAttributeMapping('bullet_point1', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'bullet_point2') {
                        $this->addAttributeMapping('bullet_point2', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'bullet_point3') {
                        $this->addAttributeMapping('bullet_point3', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'bullet_point4') {
                        $this->addAttributeMapping('bullet_point4', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'bullet_point5') {
                        $this->addAttributeMapping('bullet_point5', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'other_image_url1') {
                        $this->addAttributeMapping($column->fields, $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'other_image_url2') {
                        $this->addAttributeMapping($column->fields, $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'other_image_url3') {
                        $this->addAttributeMapping($column->fields, $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'other_image_url4') {
                        $this->addAttributeMapping($column->fields, $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'other_image_url5') {
                        $this->addAttributeMapping($column->fields, $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'sale_price') {
                        $this->addAttributeMapping('sale_price', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'sale_price_dates_from') {
                        $this->addAttributeMapping('sale_price_dates_from', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'sale_price_dates_to') {
                        $this->addAttributeMapping('sale_price_dates_to', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'parent_child') {
                        $this->addAttributeMapping('parent_child', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'parent_sku') {
                        $this->addAttributeMapping('parent_sku', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'variation_theme') {
                        $this->addAttributeMapping('variation_theme', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'relationship_type') {
                        $this->addAttributeMapping('relationship_type', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'recommended_browse_nodes') {
                        $this->addAttributeMapping('recommended_browse_nodes', $column->fields, true, false)->localized_name = $column->labels;
                    } elseif ($column->fields == 'recommended_browse_nodes1' || $column->fields == 'recommended_browse_nodes2') {
                        $this->addAttributeMapping('recommended_browse_nodes', $column->fields, true, false)->localized_name = $column->labels;
                    } else {
                        $this->addAttributeMapping($column->fields, $column->fields, false, $column->required)->localized_name = $column->labels;
                    }
                }
            }
