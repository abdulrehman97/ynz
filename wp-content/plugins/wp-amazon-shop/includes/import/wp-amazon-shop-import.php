...
class ACL_Amazon_Product_Import
{
    ...
    public function initiate_hooks() {
       ...
       if( isset($_GET['page']) && $_GET['page']=='wp-amazon-shop-products-import' && $_GET['page']=='wp-amazon-shop-basic-import'){
        ...
        //ext
        add_action('wp_ajax_wpas_no_pa_search_ext', array($this, 'no_pa_search_ext'));
        add_action('wp_ajax_wpas_no_pa_product_insert_ext', array($this, 'no_pa_product_insert_ext'));
    }
    function hook_import_submenu(){
        ...
        add_submenu_page(
            'wp-amazon-shop',
            'WP Amazon Shop Details Products Import (Using Chrome Extension)',
            'Extended Import',
            'import',
            'wp-amazon-shop-products-import',
            array($this, 'products_import')
        );
        add_submenu_page(
            'wp-amazon-shop',
            'WP Amazon Shop Products Import Chrome Extension Authorization ',
            'Chrome Extension',
            'import',
            'wp-amazon-shop-products-import-ext',
    ...
    public function products_import(){
        require_once ('wp-amazon-extended-import.php');
    }
    public function chrome_extension(){
        require_once( 'wpas-chrome-addon.php');
    }
    ...
    public function wpas_products_import_search(){
        ...
        $processed_products = $this->process_response($_POST['data']);
        ...
        echo wp_send_json(array('keyword' => $_POST['keyword'], 'html' => $html, 'page_num' => intval($page_num + 1), 'product_num' => count($processed_products)));
        wp_die();
    }
        ...
        foreach ($products as $product) {
            ...
            <tr>
                ...
                <td class="wpas-import-title" >
                    ...
                    <p class="field_text"><a href="<?php echo $product['DetailPageURL']; ?>?tag=<?php echo $affiliate_tag;  ?>" target="_blank" class="link_to_source product_url"><?php _e('Amazon Product page', 'wp-amazon-shop') ?></a></p>
    ...
    public function wpas_import_product_from_amazon() {
        ...
        if ($product_id) {
            ...
            if($this->wpas_capabitlity()){
                ...
                $post = array(
                    'post_author' => 1,
                    'post_content' => $title . '<br><img src="' . $image_url . '" alt="' . $title . '"><br> <br>  <a href="' . $product_url . '"  target="_blank">Product Link</a>',
                    'post_status' => "publish",
                    'post_title' => $title,
                    'post_parent' => '',
                    'post_type' => "product",
                ...
                //Update term & meta.
                wp_set_object_terms($post_id, $_POST['importing_category'], 'product_cat');
               // wp_set_object_terms($post_id, get_option('acl_wpas_category_name'), 'product_cat');
                ...
                $response['status'] = 300;
                $response['success_action'] = '<a style="color: orangered" href="https://www.wpamazonshop.com/" target="_blank">'.__('Upgrade to Pro for more', 'wp-amazon-shop').'</a> ';
    ...
    private function import_operation($product){
        ...
        if ($product_id) {
            ...
            if($this->wpas_capabitlity()) {
                ...
                $response['status'] = 300;
                $response['sku'] = $product['ASIN'];
                $response['message'] = __('20 Products can be imported per day, <br>Need to import UNLIMITED,VARIATIONS,GALLERY etc <a href="https://www.wpamazonshop.com/" target="_blank">Upgrade to Pro</a> ', 'wp-amazon-shop');
            }
        }
        return $response;
    }
    public function no_pa_search_ext(){
        $search_result=$_POST['search_result'];
        $results=ACL_WPAS_iKits::no_pa_search($search_result,'ext');
        $html="";
        if(is_array($results['products']) && !empty($results['products'])){
            foreach ($results['products'] as $product){
                $html.=ACL_WPAS_iKits::search_item_display($product);
            }
        }else{
            $html=__('<strong style="color:red;text-align:center">No Products Found</strong>','wp-amazon-shop');
            $results['start']=0;
            $results['end']=0;
        }
        echo wp_send_json(array('html'=>$html,'start'=>$results['start'],'end'=>$results['end']));

    }
    public function no_pa_product_insert_ext(){
        $product=array();
        $product['ASIN']= $_POST['asin'];
        //if(ACL_WPAS_Help::get_http_status($product_url)!='404'){
        $html = new simple_html_dom();
        $html->load(urldecode(base64_decode($_POST['details'])));
        if(ACL_WPAS_iKits::get_product_title($html)!=""){
            $product['Title']= ACL_WPAS_iKits::get_product_title($html);
            if(isset($_POST['price']) && $_POST['price']!=""){
                $product['price']=$_POST['price'];
            }else{
                $product['price']=$this->price_by_asin($_POST['asin']);
            }

            $product['thumb']= $_POST['thumb'];
            //products gallery
            $product_gallery=ACL_WPAS_iKits::get_product_image_gallery($html);
            $product['gallery']= base64_encode(serialize(ACL_WPAS_iKits::build_image_gallery($product_gallery)));
            $product['Description']=base64_decode(ACL_WPAS_iKits::get_product_description($html));
            if(isset($_POST['category']) && $_POST['category']!=""){
                $product['category']= $_POST['category'];
            }else{
                $product['category']= ACL_WPAS_iKits::get_product_categories($html);
            }
            $product['has_variation']= false;
            $product['variations']=array();
            $response=$this->import_operation($product);
        }else{
            $response=array('status'=>300,'sku'=>$product['ASIN'],'message' => __('Is not valid set of data', 'wp-amazon-shop'));
        }
        echo wp_send_json($response);
        wp_die();
    }
