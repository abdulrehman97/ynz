...
class WPLA_WooBackendIntegration extends WPLA_Core {
	...
	function wpla_woocommerce_edit_product_columns($columns){
		...
		$columns['listed_on_amazon'] = '<img src="'.WPLA_URL.'/img/amazon-16x16.png" data-tip="'.__( 'Amazon', 'wp-lister-for-amazon' ).'"  class="tips" />';
		return $columns;
	}
    ...
    public function handle_product_stock_update( $product ) {
        ...
        do_action( 'wpla_product_has_changed', $id );
    }
				...
				if ( $amazon_order_id ) {
			        ...
			        if ( $order ) {
				        ...
				        if ( is_object( $order_details ) ) {
                            ...
                            // check for Business
                            if ( isset( $order_details->IsBusinessOrder ) && $order_details->IsBusinessOrder === 'true' ) {
