...
class WPLA_AmazonFeed {
	...
	function submit() {
		...
		$result = $api->submitFeed( $this->FeedType, $feed_content, $this->feedOptions, $this->MarketplaceIdList );
		// echo "<pre>";print_r($result);echo"</pre>";die();
		...
		if ( $result->success ) {
			...
			if ( ! $this->isCheckFeed() && '_POST_FLAT_FILE_FULFILLMENT_DATA_' != $this->FeedType ) {
	...
	static function updatePendingFeedForAccount( $account ) {
		...
		WPLA_AmazonFeed::buildFeed( '_POST_FLAT_FILE_INVLOADER_DATA_', $items, $account, false, false, 'ProductRemoval' );
	...
	static function buildFeed( $feed_type, $items, $account, $profile = false, $append_feed = false, $template_type = false ) {
		...
		if ( $template_type == 'ProductRemoval' ) {
			$feed_type = '_POST_FLAT_FILE_INVLOADER_DATA__PRODUCT_REMOVAL_';
		}
		...
		switch ( $feed_type ) {
			...
			case '_POST_FLAT_FILE_INVLOADER_DATA__PRODUCT_REMOVAL_':
    ...
    static function buildInvoiceUploadFeed( $order, $invoice_file_path, $invoice_id, $account_id ) {
        ...
        $amazon_order_id = get_post_meta( $order->get_id(), '_wpla_amazon_order_id', true );
        ...
        $new_feed->FeedType             = '_UPLOAD_VAT_INVOICE_';
