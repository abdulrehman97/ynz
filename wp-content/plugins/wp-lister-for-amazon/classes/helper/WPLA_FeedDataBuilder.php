...
class WPLA_FeedDataBuilder {
	...
	static function parseProductColumn( $column, $item, $product, $profile ) {
		...
		switch ( $column ) {
            ...
            case 'standard_price':
                ...
                // send empty price if external_repricer flag is set
           		$x_repricer = get_post_meta( $product_id, '_amazon_external_repricer', true );		// better performance
           		if ( $x_repricer == '1' ) $value = '';
            ...
            case 'generic_keywords': // Single search keywords field
                $value = self::doTranslate( get_post_meta( $product_id, '_amazon_search_term', true ), $profile->account_id ); break;
			...
			case 'size_map':
				if ( $product_type == 'variation' ) {
					...
					if ( $size_name && array_key_exists( $size_name, $variation_size_map ) ) {
						$value = $variation_size_map[ $size_name ];
					}
				}
				break;
		...
		$parent_var_columns = apply_filters( 'wpla_allowed_parent_var_columns', array(
            ...
            'external_product_id',
            'external_product_id_type', // added for #37997
		), $column, $item, $profile );
